#!/usr/bin/env python3
"""
AI Council Main Stable v2 - Robust Desktop Switching & Reply Forwarding
FIXED: Line 89 syntax error corrected
"""
...
AI_SEQUENCE = [
    ...
    {"name": "perplexity", "desktop": 2, "coords": (1200, 960), "read_region": (462, 171, 638, 551)},
    {"name": "finn", "desktop": 2, "coords": (1200, 960), "read_region": (462, 171, 638, 551)},
]
...
def switch_desktop(number):
    ...
    if current_desktop == number:
        return
    if number == 0:
        print("🚫 Attempt to switch to desktop 0 prevented.")
        return
    key_code = 17 + number
    script = f'tell application "System Events" to key code {key_code} using control down'
    subprocess.run(["osascript", "-e", script])
    current_desktop = number
    print(f"🖥 Switched to desktop {number}")
    time.sleep(1.5)
...
def main():
    ...
    while True:
        for ai in AI_SEQUENCE:
            session.speak(ai)
        print("🔄 Loop complete. Waiting before restarting...\n")  # FIXED: Properly escaped newline
