...
def test_full_council_flow():
    """Test complete council flow: Kai → Claude → Discussion"""
    
    logging.info("🏛️ TESTING FULL COUNCIL FLOW")
    
    # Step 1: Get Kai's response (we know this works now!)
    logging.info("📤 Step 1: Getting Kai's council theme...")
    
    kai_smart_desktop_switch("Kai")
    kai_safe_click("Kai")
    
    theme_prompt = "Kai, please set a theme for our AI Council discussion. What important topic should we explore together?"
    
    success = kai_text_injection(theme_prompt, "Kai")
    
    if not success:
        logging.error("❌ Failed to send prompt to Kai")
        return
    
    logging.info("⏳ Waiting for Kai's theme response...")
    time.sleep(12)
    
    # Step 2: Copy Kai's response using our proven OCR method
    kai_response = kai_ocr_rubberband_copy("Kai")
    
    if not kai_response:
        logging.error("❌ Failed to get Kai's theme")
        return
    
    logging.info(f"✅ Got Kai's theme: '{kai_response[:80]}...'")
    
    # Step 3: Send Kai's theme to Claude for perspective
    logging.info("📤 Step 2: Sending Kai's theme to Claude...")
    
    kai_smart_desktop_switch("CLAUDE")
    kai_safe_click("CLAUDE")
    
    claude_prompt = f"Kai has set this theme for our AI Council: '{kai_response}' Please share your perspective on this topic."
    
    success = kai_text_injection(claude_prompt, "CLAUDE")
    
    if success:
        logging.info("✅ Kai's theme sent to Claude!")
        logging.info("🎯 Council discussion initiated!")
        
        # Wait to see Claude's response
        time.sleep(8)
        
        # Could also copy Claude's response here for next step
        claude_response = kai_ocr_rubberband_copy("CLAUDE")
        
        if claude_response:
            logging.info(f"✅ Got Claude's perspective: '{claude_response[:80]}...'")
            return {"kai_theme": kai_response, "claude_perspective": claude_response}
        else:
            logging.warning("⚠️ Couldn't capture Claude's response")
            return {"kai_theme": kai_response}
    else:
        logging.error("❌ Failed to send to Claude")

def main():
    """Test the complete council flow"""
    logging.info("🚀 COMPLETE AI COUNCIL FLOW TEST")
    
    try:
        result = test_full_council_flow()
        if result:
            logging.info("🏛️ COUNCIL FLOW SUCCESSFUL!")
            logging.info(f"📊 Results: {len(result)} successful exchanges")
        else:
            logging.error("❌ COUNCIL FLOW FAILED")
    except Exception as e:
